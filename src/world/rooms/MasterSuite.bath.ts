...
import { addStatic, hinged, lightSwitch, pickup, plant, recessedLight, Toggle } from '../Props';
import { FLOOR, CEIL, FACE, drapedTowel, foldedStack, foldedTowel, placeStatic, toiletryBottle, type BottleKind } from './MasterSuite.shared';
...
// shower footprint (south-west corner)
const SH_X1 = -6.95, SH_Z1 = -0.54;
...
function doubleVanity(ctx: Ctx, x: number, z: number, rotY: number) {
  ...
  // counter clutter: toothbrush cup, tray with a candle, folded hand towel
  ...
  const tray = Prim.rbox(0.3, 0.014, 0.18, 0.005, mats.solid(0xd9d2c4, { roughness: 0.5, metalness: 0.2 })); tray.position.set(0.74, H + 0.007, 0.08); g.add(tray);
  const candle = Prim.cylinder(0.035, 0.035, 0.07, mats.solid(0xe9e2d0, { roughness: 0.5, physical: true, clearcoat: 0.3 }), { segments: 14 }); candle.position.set(0.68, H + 0.049, 0.06); g.add(candle);
  const towel = foldedTowel(ctx, 0.22, 0.14, 0xdfd8ca, 0.035); towel.position.set(0.0, H + 0.0175, 0.15); towel.rotation.y = 0.1; g.add(towel);
...
function sconce(ctx: Ctx, x: number, y: number, z: number, rotY: number, group: string) {
  ...
  const sp = new THREE.Vector3(0, 0.0, 0.11).applyAxisAngle(new THREE.Vector3(0, 1, 0), rotY).add(new THREE.Vector3(x, y, z));
  shade.position.copy(sp);
  ctx.dynamic.add(shade);
  const lp = new THREE.Vector3(0, 0.0, 0.16).applyAxisAngle(new THREE.Vector3(0, 1, 0), rotY).add(new THREE.Vector3(x, y, z));
...
function freestandingTub(ctx: Ctx, x: number, z: number, rotY: number) {
  ...
  g.add(shell);
  const overflow = Prim.torus(0.03, 0.006, mats.chrome); overflow.rotation.x = Math.PI / 2; overflow.position.set(0, 0.42, -0.6); g.add(overflow);
  ...
  const caddy = Prim.rbox(0.92, 0.02, 0.2, 0.006, mats.walnut); caddy.position.set(0, 0.6, 0.3); g.add(caddy);
  for (const s of [-1, 1]) { const lip = Prim.box(0.92, 0.03, 0.012, mats.walnut); lip.position.set(0, 0.61, 0.3 + s * 0.094); g.add(lip); }
  const candle = Prim.cylinder(0.03, 0.03, 0.06, mats.solid(0xe9e2d0, { roughness: 0.5, physical: true, clearcoat: 0.3 }), { segments: 14 }); candle.position.set(-0.3, 0.64, 0.3); g.add(candle);
  const cloth = foldedTowel(ctx, 0.16, 0.12, 0xb9c6d1, 0.03); cloth.position.set(0.0, 0.625, 0.3); cloth.rotation.y = 0.15; g.add(cloth);
  const book = Prim.rbox(0.15, 0.03, 0.2, 0.004, mats.solid(0x6b4f3a, { roughness: 0.6 })); book.position.set(0.28, 0.625, 0.3); book.rotation.y = -0.1; g.add(book);
...
function tubFiller(ctx: Ctx, x: number, z: number, rotY: number) {
  ...
  const bend = Prim.torus(0.11, 0.016, chrome, { arc: Math.PI / 2 }); bend.rotation.set(0, 0, 0); bend.rotation.x = Math.PI / 2; bend.rotation.z = 0; bend.position.set(0, 0.91, 0.11); g.add(bend);
...
/** Toilet with a working flush: static body, a dynamic lever and water surface. Front +z, tank at -z. */
class Toilet implements Interactable {
  ...
  constructor(private ctx: Ctx, private lever: THREE.Object3D, private water: THREE.Mesh, focus: THREE.Vector3) {
    this.object = new THREE.Group();
    this.object.add(lever, water);
    this.focus = focus;
  ...
  update(dt: number) {
    ...
    if (t > 4.5) { this.t = -1; this.water.scale.set(1, 1, 1); this.water.position.y = this.baseY; this.lever.rotation.x = 0; }
  }
}
...
function toilet(ctx: Ctx, x: number, z: number, rotY: number) {
  ...
  const seat = Prim.torus(0.205, 0.028, mats.solid(0xf7f7f3, { roughness: 0.3, physical: true, clearcoat: 0.6 })); seat.scale.set(1, 1, 1.3); seat.position.set(0, 0.415, 0.06); g.add(seat);
  const lid = Prim.rbox(0.42, 0.025, 0.5, 0.012, mats.solid(0xf7f7f3, { roughness: 0.3, physical: true, clearcoat: 0.6 })); lid.position.set(0, 0.62, -0.13); lid.rotation.x = -1.35; g.add(lid);
  const tank = Prim.rbox(0.46, 0.38, 0.2, 0.012, mats.ceramic); tank.position.set(0, 0.6, -0.28); g.add(tank);
  const tankLid = Prim.rbox(0.48, 0.03, 0.22, 0.006, mats.ceramic); tankLid.position.set(0, 0.805, -0.28); g.add(tankLid);
  const hinge = Prim.box(0.2, 0.02, 0.05, mats.chrome); hinge.position.set(0, 0.42, -0.19); g.add(hinge);
  const supply = Prim.cylinder(0.006, 0.006, 0.25, mats.chrome, { segments: 6 }); supply.position.set(-0.2, 0.2, -0.33); g.add(supply);
  placeStatic(ctx, g, x, z, rotY, [{ size: [0.48, 0.82, 0.75], center: [0, 0.41, -0.06] }], 'tile');
  // dynamic: lever + water
  g.updateMatrixWorld(true);
  ...
  lv.position.copy(new THREE.Vector3(-0.17, 0.74, -0.18).applyMatrix4(g.matrixWorld)); lv.rotation.y = rotY;
  ctx.dynamic.add(lv);
  const water = Prim.cylinder(0.12, 0.12, 0.004, mats.water, { segments: 20, cast: false, receive: false });
  water.scale.z = 1.3;
  water.position.copy(new THREE.Vector3(0, 0.235, 0.06).applyMatrix4(g.matrixWorld)); water.rotation.y = rotY; water.renderOrder = 11;
  ctx.dynamic.add(water);
  const focus = new THREE.Vector3(0, 0.6, 0).applyMatrix4(g.matrixWorld);
  ctx.interact.add(new Toilet(ctx, lv, water, focus));
}
...
function showerEnclosure(ctx: Ctx) {
  ...
  rail(w, true, cx, 0.075, z1); rail(w, true, cx, H, z1);
  rail(d, false, x1, 0.075, cz); rail(d, false, x1, H, cz);
  ...
  const fixedD = 0.42;
  const side = Prim.box(0.008, H - 0.1, fixedD, glassM, { cast: false, receive: false }); side.position.set(x1, y + H / 2 + 0.03, z0 + fixedD / 2 + 0.01); side.renderOrder = 10; ctx.dynamic.add(side);
  ctx.physics.addBox({ x: cx, y: y + H / 2, z: z1 }, { x: w, y: H, z: 0.03 }, 0, { meta: { surface: 'glass' } });
  ctx.physics.addBox({ x: x1, y: y + H / 2, z: z0 + fixedD / 2 + 0.01 }, { x: 0.03, y: H, z: fixedD + 0.02 }, 0, { meta: { surface: 'glass' } });
  // hinged door (opens outward into the room), with a kinematic collider that follows the leaf
  const zh = z0 + fixedD + 0.03;
  const leafL = z1 - 0.02 - zh;
  const root = new THREE.Group(); root.position.set(x1, y, zh); ctx.dynamic.add(root);
  ...
  const hp = hinged(ctx, root, new THREE.Vector3(0, 0, 0), (pivot) => {
    ...
    const top = Prim.box(0.03, 0.03, leafL, chrome); top.position.set(0, H, leafL / 2 + 0.005); leaf.add(top);
    const bot = Prim.box(0.03, 0.03, leafL, chrome); bot.position.set(0, 0.075, leafL / 2 + 0.005); leaf.add(bot);
    const hingeBar = Prim.box(0.03, H - 0.06, 0.03, chrome); hingeBar.position.set(0, H / 2 + 0.03, 0.01); leaf.add(hingeBar);
    for (const s of [-1, 1]) {
      const hb = Prim.cylinder(0.009, 0.009, 0.26, chrome, { segments: 10 }); hb.position.set(s * 0.045, 1.05, leafL - 0.07); leaf.add(hb);
      for (const dy of [-0.1, 0.1]) { const stub = Prim.cylinder(0.007, 0.007, 0.045, chrome, { segments: 8 }); stub.rotation.z = Math.PI / 2; stub.position.set(s * 0.0225, 1.05 + dy, leafL - 0.07); leaf.add(stub); }
    }
  ...
  // mixer (dynamic toggle target) on the west wall
  const mg = new THREE.Group();
  const plate = Prim.rbox(0.012, 0.14, 0.14, 0.004, chrome); mg.add(plate);
  const knob = Prim.cylinder(0.03, 0.03, 0.05, chrome, { segments: 16 }); knob.rotation.z = Math.PI / 2; knob.position.set(0.03, 0.02, 0); mg.add(knob);
  const stalk = Prim.rbox(0.03, 0.012, 0.08, 0.004, chrome); stalk.position.set(0.055, 0.02, -0.03); mg.add(stalk);
  const diverter = Prim.cylinder(0.018, 0.018, 0.03, chrome, { segments: 12 }); diverter.rotation.z = Math.PI / 2; diverter.position.set(0.02, -0.045, 0); mg.add(diverter);
  const mixer = mergeByMaterial(mg);
  mixer.position.set(x0 + 0.006, y + 1.15, cz + 0.12);
...
function towelHook(ctx: Ctx, x: number, y: number, z: number, rotY: number, color: number) {
  ...
  const hook = Prim.torus(0.02, 0.006, mats.chrome, { arc: Math.PI }); hook.rotation.set(0, Math.PI / 2, 0); hook.position.set(0, 0, 0.03); g.add(hook);
...
function towelStool(ctx: Ctx, x: number, z: number) {
  ...
  const g = new THREE.Group();
  const wood = mats.oak;
  const top = Prim.cylinder(0.17, 0.17, 0.03, wood, { segments: 20 }); top.position.y = 0.435; g.add(top);
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * Math.PI * 2 + 0.5;
    const leg = Prim.cylinder(0.014, 0.02, 0.43, wood, { segments: 8 });
    leg.position.set(Math.sin(a) * 0.12, 0.215, Math.cos(a) * 0.12);
    leg.rotation.set(Math.cos(a) * 0.12, 0, -Math.sin(a) * 0.12);
    g.add(leg);
  }
  const stack = foldedStack(ctx, [0xdfd8ca, 0x9fb7c9, 0xf2f2ec], 0.28, 0.22, 0.05); stack.position.set(0, 0.45, 0); g.add(stack);
  placeStatic(ctx, g, x, z, ctx.rng() * Math.PI, [{ size: [0.36, 0.6, 0.36], center: [0, 0.3, 0] }]);
}
...
function paperStand(ctx: Ctx, x: number, z: number, rotY: number) {
  ...
  const roll = Prim.cylinder(0.055, 0.055, 0.1, mats.solid(0xf7f6f2, { roughness: 0.9 }), { segments: 16 }); roll.rotation.z = Math.PI / 2; roll.position.set(0.07, 0.6, 0); g.add(roll);
  ...
  const spare = Prim.cylinder(0.055, 0.055, 0.1, mats.solid(0xf7f6f2, { roughness: 0.9 }), { segments: 16 }); spare.position.set(0, 0.065, 0); g.add(spare);
...
export function buildMasterBath(ctx: Ctx) {
  ...
  // ---- lighting ------------------------------------------------------------------------------
  recessedLight(ctx, -7.4, CEIL, -0.99, 'masterbath');
  ...
  toiletryPickup(ctx, 'pump', 0xf1ede4, -5.43, y + 0.86, 0.1, 'hand soap', 0xc9c9c4);
  toiletryPickup(ctx, 'pump', 0xb9c6d1, -5.3, y + 0.86, 0.98, 'lotion', 0xf2f2ef);
  toiletryPickup(ctx, 'jar', 0xe8dcc8, -5.46, y + 0.86, 0.9, 'face cream', 0xd9c7a3);
  plant(ctx, -5.2, y + 0.86, vanityZ, 0.36, { potColor: 0xe8e0d0 });
  ...
  tubFiller(ctx, -6.93, 0.3, FACE.negX);
  {
    ...
    mat.position.set(-6.72, 0.009, 0.55);
    place(mat, -6.72, y + 0.009, 0.55, 0);
    addStatic(ctx, mat);
  }
  plant(ctx, -7.8, y + 1.35, 0, 0.38, { potColor: 0xf1ede4 });

  // ---- south-west: shower; south wall: toilet + paper stand ------------------------------------------
  showerEnclosure(ctx);
  toilet(ctx, -6.05, SOUTH + TILE_T + 0.39, FACE.posZ);
  paperStand(ctx, -5.6, -1.2, 0.3);

  // ---- towels, stool ----------------------------------------------------------------------------------
  towelBar(ctx, -7.42, y + 1.3, NORTH - TILE_T, FACE.negZ, 0.64, [[-0.15, 0.26, 0xdfd8ca], [0.16, 0.26, 0x9fb7c9]]);
  towelHook(ctx, -5.55, y + 1.78, NORTH, FACE.negZ, 0xdfd8ca);
  towelStool(ctx, -5.38, 1.22);
}
